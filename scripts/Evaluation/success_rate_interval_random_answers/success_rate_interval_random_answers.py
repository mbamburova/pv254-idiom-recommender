...
def success_rate_interval_random_answers():
    data = pd.read_csv('user_answers_random_answers.csv', sep=';')
    output = pd.DataFrame(columns=['interval','rate'])
    total = len(data.index)
    intervals = math.ceil(total/50)
    for i in range(intervals):
        intervals_rate = pd.read_csv('user_answers_random_answers.csv', sep=';', skiprows= range(1, i * 50), nrows=50)
    ...
    ax.set_title("Interval success rate for random answers", size = 50)
    ...
    fig.savefig("./success_rate_interval_random_answers.pdf")

success_rate_interval_random_answers()
